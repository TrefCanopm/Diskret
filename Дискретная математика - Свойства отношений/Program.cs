...
namespace Дискретная_математика___Свойства_отношений
{
    internal class Program
    {
        ...
        void ProcessingString(string str, ref bool[ , ] matrix, int i)
        {
            ...
            {
                Console.WriteLine("Совподений не найденно");
            }
        }
        void WritingDataFile(ref bool [ , ] matrix)
        {
            StreamReader file = new StreamReader("D:\\Мои проекты Vethyal studia\\Дискретная математика - Свойства отношений\\Matrix.txt");
        ...
        void CheckReflex(ref bool[,] matrix, ref bool isChekReflex)
        {
            ...
            if (isChange) 
            {
                if (isChekReflex)
                {
                    Console.WriteLine("1) Матрица рефлексивная");
                }
                else
                {
                    Console.WriteLine("1) Матрица антирефлексивная");
                }
            }
            else
            {
                Console.WriteLine("1) Не выполняеться");
                isChekReflex = true;
            }
        }
        ...
        void ChekSymmetry(ref bool[,] matrix, ref bool isChekReflex) // При 7 или 6 не прочитывает смену
        {
            ...
            if(!isChange)
            {
                Console.WriteLine("2) Не выполняеться");
            }
            else
            {
                if (isChekSymmetry && isChekReflex)
                {
                        Console.WriteLine("2) Матрица семетричная");
                }
                else
                {
                    if (isChekReflex)
                    {
                        Console.WriteLine("2) Матрица антисеметричная");
                    }
                    else
                    {
                        Console.WriteLine("2) Матрица асеметриная");
                    }
                }
            }
        }
        ...
        void ChekTransitivity(ref bool[,] matrix)
        {
            ...
            {
                Console.WriteLine("3) Матрица не тразитивная");
            }
        }
